...
export const createProductSchema = z.object({
  body: z.object({
    ...
    onPaperRate: z.number().min(0, 'On paper rate must be a positive number').max(999999.99, 'On paper rate too high').optional(),
...
export const updateProductSchema = z.object({
  body: z.object({
    ...
    onPaperRate: z.number().min(0, 'On paper rate must be a positive number').max(999999.99, 'On paper rate too high').nullable().optional(),
    discount: z.number().min(0, 'Discount cannot be negative').max(100, 'Discount cannot exceed 100%').optional(),
  }),
  query: z.object({}).optional(),
  params: z.object({}).optional(),
